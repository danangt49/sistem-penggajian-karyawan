                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h3 class="card-title">Data Tunjangan Skill</h3>
                                        @if (Auth::user()->level == 'admin')
                                            <a href="{{ url('master/tunjangan-skill/form') }}"
                                                class="btn btn-primary float-right"><i class="fas fa-plus"></i> Tambah</a>
                                        @endif

                                        <a href="{{ url('master/tunjangan-skill-cetak-all-pdf') }}"
    ...
    <script>
        $(document).ready(function() {
            $('#datatable').dataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ url('master/json-tunjangan-skill') }}",
                columns: [{
                        title: 'No',
                        data: null,
                    ...
                    {
                        title: 'Nama',
                        data: 'nm_tunjangan_skill'
                    },
                    {
                        title: 'Nominal (Rp.)',
                        data: 'jumlah_tunjangan_skill'
                    },
                    {
                        title: 'Keterangan',
                        data: 'keterangan'
        ...
        $('body').on('click', '.delete', function() {
            ...
            var document_kd_tunjangan = $(this).data("kd_tunjangan_skill");
            swal({
                ...
                .then((result) => {
                    if (result.value) {
                        $.ajax({
                            url: "{{ url('master/tunjangan-skill-delete') }}" + '/' +
                                document_kd_tunjangan,
                            type: "POST",
                            data: {
                                '_method': 'GET',
                                '_token': csrf_token
