                                ...
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        @if (Auth::user()->level == 'admin')
                                            <a href="{{ url('master/lembur/form') }}" class="btn btn-primary float-right"><i
                                                    class="fas fa-plus"></i> Tambah</a>
                                        @endif

                                        <a href="{{ url('master/lembur-cetak-all-pdf') }}"
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            $('#datatable').dataTable({
                ...
                columns: [{
                    ...
                    {
                        title: 'Biaya Perjam (Rp.)',
                        data: 'biaya_lembur_perjam'
                    },
                    {
                        title: 'Total',
                        data: 'total_pendapatan_lembur'
                    },
                    @if (Auth::user()->level == 'admin')
                        {
                            title: 'Aksi',
        ...
        $('body').on('click', '.delete', function() {
            ...
            var document_kd_lembur = $(this).data("kd_lembur");
            swal({
                ...
                .then((result) => {
                    if (result.value) {
                        $.ajax({
                            url: "{{ url('master/lembur-delete') }}" + '/' + document_kd_lembur,
                            type: "POST",
                            data: {
                                '_method': 'GET',
                                '_token': csrf_token
